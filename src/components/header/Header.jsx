...
function Header() {
  ...
  return (
    <div className='flex items-center p-2'>
      <img src={logo} alt="logo" style={{ height: "4rem", width: "5rem" }} />
      ...
      <div>
      {theme === 'light' ? (
          <HiMoon onClick={() => setTheme('dark')} className='text-4xl bg-slate-200 text-black rounded-full p-1 cursor-pointer' /> 
        ) : (
          <HiSun onClick={() => setTheme('light')} className='text-4xl bg-[#11291f] text-white rounded-full p-1 cursor-pointer' />
        )}
      </div>
    </div>
  )
