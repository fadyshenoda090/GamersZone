...
function Header() {
  ...
  return (
    ...
    {searchResults.length > 0 && searchQuery.length!=0 && (
      <div className="z-40 search-results absolute left-[7.5rem] right-0 top-12 bg-white rounded-lg mt-2 w-[88%]">
        {searchResults.map((result) => (
          <div key={result.id} className="p-2 border-b cursor-pointer RockSalt">{result.name}</div>
        ))}
      </div>
    )}
  </div>
