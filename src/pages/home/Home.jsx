...
const Home = () => {
  ...
  const getGamesByGenre = async (id) => {
    try {
      const res = await axiosInstance.get(`/games?genres=${id}`);
  ...
  return (
    <div>
      {list.length > 0 && randomGames.length > 0 && GenreGamesList.length > 0 ? (
        <div className='grid grid-cols-4 px-5'>
          <div className='h-full hidden md:block'>
          <GenresList genreId={(setGenreId)=>getGamesByGenre(setGenreId)}/>
          </div>
        ...
        <div className='min-h-screen flex justify-center items-center'>
          <div className='flex justify-center items-center'>
            <div role="status">
              <svg aria-hidden="true" className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-100 fill-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
